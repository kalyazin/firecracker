...
fn override_clidr(vcpus: &[Vcpu]) -> Result<(), ConfigurationError> {
    ...
    if let Err(e) = vcpus[0]
        .kvm_vcpu
        .fd
        .get_one_reg(regs::CLIDR_EL1, &mut cur_bytes)
    {
        logger::warn!("Failed to read CLIDR_EL1, skipping override: {e}");
        return Ok(());
    }

    let cur_clidr = u64::from_le_bytes(cur_bytes);
    ...
    if new_clidr != cur_clidr {
        ...
        for vcpu in vcpus.iter() {
            if let Err(e) = vcpu.kvm_vcpu.fd.set_one_reg(regs::CLIDR_EL1, &new_bytes) {
                logger::warn!(
