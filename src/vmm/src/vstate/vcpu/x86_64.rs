...
impl KvmVcpu {
    ...
    pub fn save_state(&self) -> Result<VcpuState> {
        ...
        for chunk in msr_list.chunks(KVM_MAX_MSR_ENTRIES) {
            let mut msrs = Msrs::new(chunk.len()).map_err(Error::FamError)?;
        ...
        for mut msrs in all_msrs.iter_mut() {
        ...
        Ok(VcpuState {
            ...
            saved_msrs: all_msrs,
            msrs: Msrs::new(0).map_err(Error::FamError)?,
            debug_regs,
            lapic,
            mp_state,
            regs,
...
impl VcpuState {
    ...
    fn ser_saved_msrs(&mut self, target_version: u16) -> VersionizeResult<()> {
        if target_version < 3 && self.saved_msrs.len() == 1 {
            self.msrs = self.saved_msrs[0].clone();
            return Ok(());
        }
        Err(VersionizeError::Serialize(
            "Cannot serialize MSRs. The uVM state needs to save more MSRs than the target \
             snapshot version supports."
                .to_string(),
        ))
    }
}
