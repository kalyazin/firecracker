...
impl Vm {
    ...
    pub fn create_vcpus(&mut self, vcpu_count: u8, secret_free: bool) -> VcpuCreationResult {
        ...
        for cpu_idx in 0..vcpu_count {
            ...
            if secret_free {
                userfault_channels.push(vmm_channel.expect("Failed to create userfault channel"));
            }
        }
    ...
    pub fn register_memory_regions(
        ...
        userfault_bitmap: Option<&[u8]>,
    ) -> Result<(), VmError> {
        for region in regions {
            let userfault_bitmap = match userfault_bitmap {
                Some(addr) => {
                    let file_offset_start = region
                        .file_offset()
                        .expect("file offset not present")
                        .start();
                    let pages_offset = file_offset_start / (host_page_size() as u64);
                    let bytes_offset = pages_offset / (u8::BITS as u64);

                    Some(&addr[usize::try_from(bytes_offset).expect("bytes_offset is too large")..])
                }
                None => None,
            };
